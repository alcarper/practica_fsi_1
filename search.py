...
class Node:
    ...
    def __init__(self, state, parent=None, action=None, path_cost=0, heuristic_cost=0):
        ...
        update(self, state=state, parent=parent, action=action,
               path_cost=path_cost, depth=0, heuristic_cost=heuristic_cost)
        if parent:
            self.depth = parent.depth + 1

    def __repr__(self):
        return "<Node %s pc:%d h:%d>" % (self.state,self.path_cost, self.heuristic_cost)
    ...
    def expand(self, problem):
        ...
        return [Node(next, self, act,
                     ...
                     distance(getattr(problem.graph, 'locations', None)[next],
